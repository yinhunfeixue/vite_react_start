...
class AutoTip extends Component<IAutoTipProps, IAutoTipState> {
  ...
  private checkTooltip() {
    ...
    const showTooltip =
      content.offsetWidth > wrap.clientWidth ||
      content.offsetHeight > wrap.offsetHeight;
  ...
  render() {
    ...
    const contentNode = isString ? (
      <span title='' dangerouslySetInnerHTML={{ __html: content }} />
    ) : (
    ...
    const contentElement = (
      <div
        onClick={onClick}
        ref={this.wrapRef}
        style={{ ...style, WebkitLineClamp: maxLine }}
        className={classNames('AutoTip', className)}
      >
